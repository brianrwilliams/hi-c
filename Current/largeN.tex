...
\] 
\item[(3)] The $U(d)$-invariant subcomplex of the right-hand side is equal to $\left(\CC[t] / \CC\right) [2d]$ where $t$ is a formal variable of degree $+2$. 
...
Step (3) is a consequence of the Hochschild-Kostant-Roesenberg theorem applied to the cyclic homology of the ring $\CC[[z_1,\ldots,z_d]]$.
If $t$ denotes a formal variable of degree $2$, there is a quasi-isomorphism 
\[
{\rm Cyc}_*(\CC[[z_1,\ldots,z_d]]) \simeq \left(\CC[[z_i]][\d z_i] [t^{-1}], t \d_{dR}\right)
\]
where the $\d z_i$ have degree $-1$.
In turn, Step(3) follows from the formal Poincar\'{e} lemma applied to $\hD^d$ which implies there is a quasi-isomorphism
\[
