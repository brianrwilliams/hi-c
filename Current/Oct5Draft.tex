...
This map 
\[
\fj_{\CC^d} : \Sym^{d+1}(\fg^*)^\fg [-1] \xto{\simeq} \left(\cloc^*(\sG({\CC^d}))^{\CC^{2d|d}}\right)^{U(d)}
\]
is a quasi-isomorphism.
\end{prop}

As the proof is rather lengthy, we provide it in Appendix B.
\owen{Make this not hardcoded}
...
\section{Computing the deformation complex}
