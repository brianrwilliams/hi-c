...
\begin{dfn}
For a local Lie algebra $(L\to X, \ell_1,\ell_2)$, its precosheaf $\sL$ of {\em linear currents} is given by taking compactly supported sections of~$L$.
\end{dfn}

We use the term ``linear'' here because the product of two such currents is not in $\sL$ itself, 
although such a product will exist in the larger precosheaf of observables.

\begin{rmk}
This precosheaf of dg Lie algebras ``forgets'' to a cosheaf of cochain complexes.
(The coproduct of dg Lie algebras is not the direct sum of cochain complexes, so $\sL$ is not a cosheaf of dg Lie algebras.)
It forms, however, a factorization algebra with values in dg Lie algebras,
since the forgetful functor from dg Lie algebras to cochain complexes preserves
\end{rmk}
