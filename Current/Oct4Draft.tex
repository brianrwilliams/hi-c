...
\begin{proof} 
\owen{I'm not sure what's the best way to write a proof. Currently the key explicit computations aren't actually done, so I might be inclined to write "direct computation shows \dots" Alternatively we just do the computations. What you wrote somehow feels in between: not enough for those who need details and more than needed for those who know what's going on.}
...
Hence the differential on the local cochains of $\sG_X$ splits as a sum $\dbar + \d_{\fg}$ where $\dbar$ denotes the differential on functionals  induced from the $\dbar$ differential on the Dolbeault forms and $\d_{\fg}$ is induced from the Lie bracket on $\fg$. 
We observe that
\begin{align*}
\dbar \fj_X(\theta) & = 0 \\
\d_{CE} \fj_X(\theta) & = 0 .
...
\brian{This shouldn't go here anymore. Not really a ``global aspect" of the KM.}
