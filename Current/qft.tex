...
\owen{Let's be even more precise if we can.}
...
In terms of this convention, the commutator in $U(\sH_{d,V})$ can be described by contractions with the propagator $P = P_{0 < \infty}$ for $\Omega^{0,*}(\CC^d)$ restricted to the subcomplex $A_d$. 
\owen{I think this is the crux of the matter: we don't explain why the propagator is relevant.}
...
We can now describe explicitly a simple linear operator in $U(\sH_{d,V})$ via residues.   
...
Let us now examine what we can say in the case~$d=2$.
The propagator is then given by
\[
P (z,\zbar) = \frac{1}{(2\pi i)^d} \frac{\zbar_1 \d \zbar_2 - \zbar_2 \d \zbar_1}{|z|^4} \tensor \id_{V} .
\]
The vector space $V$ is a $\fg$-representation, 
Consider the two quadratic observables in $U (\sH_{d,V})$ given~by
\[
\oint_{S^3} \d^2 z\, \<b, (a \cdot c)\>_V \quad \text{ and } \quad \oint_{S^3} \d^2 z\, \<b, (a'\cdot c)\>_V 
\]
where $a,a' \in A_2 \tensor \fg$. 
By the definition of the associative product, the bracket
\beqn\label{bracket}
...
\owen{We don't really explain this. }
