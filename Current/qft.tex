...
It extends, in a natural way, to an action of the ``gauged'' Lie algebra $C^\infty(\CC^d, \fg)$ on the fields (\ref{gammafield}): an element $x(z,\zbar) \in C^\infty(X,\fg)$ acts simply by $x(z,\zbar) \cdot \gamma(z,\zbar)$ where the dot indicates the pointwise action via the $\fg$-module structure on $V$. 
This Lie algebra action is compatible with the action functional (\ref{actionfnl})---preserves solutions to the equations of motion---precisely when $x(z,\zbar)$ is holomorphic: $\dbar x(z,\zbar) = 0$. 
...
There is a $(-1)$-shifted skew-symmetric nondegenerate pairing is given by integration along $X$ combined with the evaluation pairing between $V$ and its dual: 
\[
