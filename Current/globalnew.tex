...
Fixing any Hermitian metric on $X$ we can consider the Dolbeualt Laplacian $\Delta_{\dbar} = [\dbar,\dbar^*]$ where $\dbar^*$ is the adjoint of $\dbar$.
...
\begin{tikzcd}
\displaystyle \UU_\theta (\sG) (X) = \clieu_*(\Omega^{0,*}(X , \fg)) \ar[r,"\pi^{0,*}_{\sH}", "\simeq"'] &\clieu_*(\CC[\epsilon] \tensor \fg) .
\end{tikzcd}
\eeqn
...
\[
{\rm ch}_{X,V} : HH_0(U \fg) = \Sym(\fg)^{\fg} [\hbar^{-1}] \to \CC[\hbar,\hbar^{-1}] .
\] 
This map is natural in the module $V$.
\end{prop}
