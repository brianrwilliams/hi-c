...
\owen{I rewrote the definition from your thesis' appendix to match what FHK actually write. Please double check.}
...
\owen{Notation for this? ${\Sph}_d(\fg)$?}
\end{dfn}
...
This cocycle has cohomological degree $2$ and so determines an unshifted central extension of $A_d \tensor \fg$:
\beqn\label{gdt}
\CC \cdot K \to \Hat{\fg}_{d, \theta} \to A_d \tensor \fg .
\eeqn
\owen{You wrote dg Lie but I think it must be $L_\infty$, right?}
Our aim is now to show how the Kac-Moody factorization algebra is related to this $L_\infty$ algebra,
...
by postcomposing the map $j$ of proposition~\ref{prop: Ad} with the restriction map.
We abusively denote the composite by $j$ as well.
Thus we obtain a natural map of dg commutative algebras
\[
\pi_A: \Omega^* \otimes A_d \to r_* \Omega^{0,*}
\]
sending $\alpha \otimes \omega$ to $\pi(\alpha) \wedge j(\omega)$.
...
\]
Of course, we can restrict $\sG_{J(\theta)}$ to a factorization algebra on $\CC^d \setminus 0$. The factorization algebra $\sG_2$ is defined as the pushforward of this restriction along the radial projection: $\sG_2 := r_* \left(\sG_{J(\theta)}|_{\CC^d \setminus 0}\right)$. 

Recall the map $\Phi : \Omega^*_{\RR_{>0},c} \tensor (A_d \tensor \fg) \to r_*(\Omega^{0,*}_{\CC^d \setminus 0,c} \tensor \fg)$ defined in Equation (\ref{phi map}). On each open set $I \subset \RR_{>0}$ we can extend $\Phi$ by the identity on the central element to a linear map $\Phi_2 : \sG_1' (I) \to \sG_2 (I)$. 
...
which yields a natural map \owen{ugly name so lets find a better one}
\[
...
It remains to provide the the structure maps of the putative prefactorization algebra~$\cC$.
...
Hence, for any pairwise disjoint opens $U_1,\ldots, U_n$ contained in a larger open $V$, 
the isomorphism of dg algebras
\[
\Omega^{0,*}_c(U_1) \times \cdots \times \Omega^{0,*}_c(U_n) \cong \Omega^{0,*}_c(U_1 \sqcup \cdots \sqcup U_n),
\]
which is a structure map of the cosheaf $\Omega^{0,*}_c$,
...
\eeqn
Applying $\Sym$ we obtain the needed multilinear map
\[
\cC(U_1) \times \cdots \times \cC(U_n) \to \cC(V)
\]
as the composite 
\[
\cC(U_1) \times \cdots \times \cC(U_n) \to \cC(U_1 \sqcup \cdots \sqcup U_n) \to \cC(V)
\]
where the first map is $\Sym$ applied to the map \eqref{eqn:cyccosheaf} 
and the second map is $\Sym$ applied to
\[
\cycls(\Omega^{0,*}_c(U_1 \sqcup \cdots \sqcup U_n)) \to \cycls(V),
\]
from the precosheaf $\cycls(\Omega^{0,*}_c)$ on the inclusion of opens $U_1 \sqcup \cdots \sqcup U_n \to V$.
\end{proof}
