...
When the twisting cocycle defining the Kac-Moody factorization algebra is zero we will see that the $E_d$ algebra coming from the product of polyannuli is equivalent to the $E_d$-enveloping algebra of the torus algebra $U^{E_d} (L^d \fg)$.
...
This factorization algebra on $(\RR_{>0})^d$ is defined by the pushforward $\vec{r}_*\left(\sG_d\right)$, 
where $\vec{r} : (\CC^\times)^d \to (\RR_{>0})^d$ is the projection $(z_1,\ldots,z_d) \mapsto (|z_1|, \cdots, |z_d|)$ where we take the modulus in each component.
...
\begin{prop}
Fix $\theta \in \Sym^{d+1}(\fg^*)^\fg$ and let $\vec{r}_* \UU_\theta \sG_d$ be the factorization algebra on $(\RR_{>0})^d$ obtained by reducing the Kac-Moody factorization algebra on $(\CC^\times)^d$ along the real $d$-torus.
The $E_d$-algebra 
\[
U^{E_d} \left( \Hat{L^d \fg}_{\theta} \right)
\]
thought of as a factorization algebra on $(\RR_{>0})^d$ embeds densely inside $\vec{r}_* \UU_\theta \sG_d$. 
\end{prop}
