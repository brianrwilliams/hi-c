...
\begin{eg}
\brian{might be good to mention the abelian example $\sL = \Omega^*_c$ and how this returns "currents" in the ordinary sense}
\end{eg}
...
\owen{I took the full de Rham complex, but if you prefer, we can just tensor with densities.} \brian{Though, the de Rham complex is computed by the derived tensor product.}
...
\[
\Hat{\ell}_k(x_1 + a_1, \ldots, x_k + a_k) = 0+ \Theta(x_1,x_2,\ldots, x_k) \in \fg \oplus \CC[n-k] .
\]
...
\begin{rmk}
There is modest extension of the cocycle $\fj_X$ that makes sense for the local Lie algebra $\sAd(P)$,  where $P \to X$ is a principal $G$-bundle. 
When the bundle $\ad(P)$ is trivialized over an open set $U \subset X$,
