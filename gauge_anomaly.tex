...
We propose a free BV theory that one may think of as higher dimensional matter in a holomorphic setting. 
...
\ben
\sO(V) = \Sym (V^\vee) = \bigoplus_{n \geq 0} (V^\vee)^{\tensor n}_{S_n} \;\;\; , \;\;\; \Hat{\sO}(V) = \Hat{\rm Sym}(V^\vee) = \prod_{n \geq 0} (V^\vee)^{\tensor n}_{S_n} . 
\een 
